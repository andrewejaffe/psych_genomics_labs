...
# data from https://www.ncbi.nlm.nih.gov/pubmed/25879659
dir.create("data")
setwd("data")
...
sapply(paste(files, ".gz", sep=""), gunzip, overwrite=TRUE)
setwd("..")
...
## ----annotatesamples--------------------------------------------------------------------
samplenames <- substr(colnames(x), 6, 15)
samplenames
...
geneid <- rownames(x)
genes <- select(org.Mm.eg.db, keys=geneid, 
                columns=c("SYMBOL"), 
                keytype="ENTREZID")
head(genes)
...
design1 = model.matrix(~group+lane)
...
## ----camera-----------------------------------------------------------------------------
load("extdata/mouse_c2_v5p1.rda") # from the package RNAseq123
idx <- ids2indices(Mm.c2,id=rownames(v))
cam.BasalvsLP <- camera(v,idx,design,
